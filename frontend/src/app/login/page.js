...
function loginButton(){
  ...
  console.log("A new requisitation for username > ",username)
  const url = "https://animated-parakeet-97456gj46g96fp4gp-8000.app.github.dev/api/login/";
    ...
    response.then(async(res) => {
        if(res.status === 200){
            ...
            if(data.status === "success"){
                console.log(`User ${data.user.username} authenticated'!`);
...
export default function Login(){
    return (
        <div className="d-flex align-items-center justify-content-center" style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)" }}>
            <div className="form-signin shadow p-4 rounded" style={{ width: "100%", maxWidth: "400px" }}>
                ...
                <div className="d-grid gap-2">
                    <button className="btn btn-lg btn-primary btn-block" type="button" onClick={(e)=> loginButton(e)}>Entrar</button>
                </div>
            </div>
