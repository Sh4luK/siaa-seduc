...
import styles from "../../[conversaId]/page.module.css";
...
export default function ConversaResponsavelProfessorPage() {
  ...
  const carregarConversa = useCallback(async () => {
    setErro(null);
    try {
      const res = await fetch(`${API_BASE}/api/professor/mensagens/responsaveis/${conversaId}`, { credentials: "include" });
      ...
      <div className={layoutStyles.shell}>
        <aside className={`${layoutStyles.sidebar} ${menuOpen ? layoutStyles.sidebarOpen : ""}`}>
          ...
          <nav className={layoutStyles.nav}>
            <Link href="/professor" className={layoutStyles.navLink}><i className="ti ti-home" aria-hidden="true" />Início</Link>
            <Link href="/professor/turmas" className={layoutStyles.navLink}><i className="ti ti-users" aria-hidden="true" />Minhas turmas</Link>
            <Link href="/professor/calendario" className={layoutStyles.navLink}><i className="ti ti-users" aria-hidden="true" />Calendario Escolar</Link>
        ...
        {menuOpen && <button className={layoutStyles.overlay} aria-label="Fechar menu" onClick={() => setMenuOpen(false)} />}
        ...
        <div className={layoutStyles.content}>
          <header className={layoutStyles.topbar}>
            ...
            <span className={layoutStyles.topbarTitle}>Conversa com responsável</span>
          </header>
          ...
          <main className={styles.main}>
            <div className={styles.cabecalho}>
              ...
              <div>
                <h1 className={styles.titulo}>{conversa?.responsavel_nome}</h1>
                <span className={styles.alunoSubtitulo}>Aluno: {conversa?.aluno_nome}</span>
              </div>
            </div>
            ...
            {erro && (
              <div className={styles.erro}>
                {erro}
                <button onClick={carregarConversa} className={styles.tentarNovamente}>Tentar novamente</button>
              </div>
            )}

            <div className={styles.corpo}>
              {carregando && !erro && <p className={styles.estadoVazio}>Carregando mensagens...</p>}
              {!carregando && !erro && mensagens.length === 0 && <p className={styles.estadoVazio}>Nenhuma mensagem ainda.</p>}
              {!carregando && mensagens.map((msg) => {
            ...
            <form className={styles.rodape} onSubmit={handleEnviar}>
              <input type="text" value={novaMensagem} onChange={(e) => setNovaMensagem(e.target.value)} placeholder="Escreva uma mensagem..." className={styles.input} disabled={carregando} />
              <button type="submit" className={styles.enviarBtn} disabled={enviando || !novaMensagem.trim() || carregando}>
