// /responsavel/[alunoId]/advertencias/page.js
"use client";
...
export default function AdvertenciasAlunoResponsavelPage() {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        const res = await fetch(`${API_BASE}/api/responsavel/alunos/${alunoId}/advertencias`, { credentials: "include" });
        if (res.status === 403) throw new Error("Você não tem acesso aprovado a este aluno.");
        if (!res.ok) throw new Error(`Falha ao buscar advertências (status ${res.status})`);
  ...
  return (
    <div className={layoutStyles.page}>
      ...
      <div className={layoutStyles.wrapper}>
        ...
        <TabsResponsavel alunoId={alunoId} ativa="advertencias" />

        {erro ? (
          <div className={layoutStyles.erro}>{erro}</div>
        ) : (
          <>
            <h1 className={layoutStyles.title}>Advertências</h1>
              ...
              <ul className={styles.lista}>
                {advertencias.map((a) => (
                  <li key={a.id} className={styles.card}>
                    <span className={styles.tipoBadge}>Advertência</span>
                    <div className={styles.cardInfo}>
                      <p className={styles.cardTitulo}>{a.titulo}</p>
                      <p className={styles.cardDescricao}>{a.descricao}</p>
                      <p className={styles.cardData}>{a.data.split("-").reverse().join("/")} · {a.emitido_por}</p>
                      {a.is_suspensao && (
                        <p className={styles.avisoSuspensao}>
                          Suspensão: {a.data_inicio_suspensao?.split("-").reverse().join("/")} a {a.data_termino_suspensao?.split("-").reverse().join("/")}
                        </p>
                      )}
                    </div>
                  </li>
