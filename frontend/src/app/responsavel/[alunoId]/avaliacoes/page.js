// /responsavel/[alunoId]/avaliacoes/page.js
"use client";
...
export default function AvaliacoesAlunoResponsavelPage() {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        const res = await fetch(`${API_BASE}/api/responsavel/alunos/${alunoId}/avaliacoes`, { credentials: "include" });
  ...
  return (
    <div className={layoutStyles.page}>
      ...
      <div className={layoutStyles.wrapper}>
        ...
        <TabsResponsavel alunoId={alunoId} ativa="avaliacoes" />

        {erro ? (
          <div className={layoutStyles.erro}>{erro}</div>
        ) : (
          <>
            <h1 className={layoutStyles.title}>Avaliações</h1>
            <p className={layoutStyles.subtitle}>{aluno?.nome_completo} · {aluno?.turma}</p>

            {avaliacoes.length === 0 ? (
              <p className={styles.vazio}>Nenhuma avaliação cadastrada para a turma.</p>
            ) : (
                ...
                {avaliacoes.map((a) => (
                  <li key={a.id} className={styles.card}>
                    <div className={styles.cardInfo}>
                      ...
                      <span className={styles.disciplinaBadge}>{a.disciplina}</span>
                      <p className={styles.cardData}>{a.professor} · {a.data.split("-").reverse().join("/")}</p>
                    </div>
                  </li>
                ))}
              </ul>
