...
export default function ConversaResponsavelCoordenacaoPage() {
  ...
  const carregarConversa = useCallback(async () => {
    setErro(null);
    try {
      const res = await fetch(`${API_BASE}/api/coordenacao/mensagens/responsaveis/${conversaId}`, {
        credentials: "include",
      });
      if (res.status === 404) {
  ...
  // polling — atualiza a conversa a cada 3s enquanto a tela está aberta
  useEffect(() => {
      ...
      <div className={styles.content}>
        <div className={styles.cabecalho}>
          ...
          <div className={styles.avatar}>
            {conversa?.responsavel_nome?.charAt(0)?.toUpperCase() || "?"}
          </div>
          <div>
            <span className={styles.professorNome}>
              {conversa?.responsavel_nome || "Carregando..."}
            </span>
            {conversa?.aluno_nome && (
              <div className={styles.alunoSubtitulo}>Aluno: {conversa.aluno_nome}</div>
            )}
          </div>
        </div>
            ...
            mensagens.map((msg) => {
              ...
              return (
                <div key={msg.id}>
                  ...
                  <div className={éCoordenacao ? styles.bolhaEnviada : styles.bolhaRecebida}>
