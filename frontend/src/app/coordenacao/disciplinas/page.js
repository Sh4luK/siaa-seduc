...
//   async function salvarRenomeio(id) {
//     if (!novoNome.trim()) return;
//     setErro(null);
//     try {
//       const res = await fetch(`${API_BASE}/api/coordenacao/disciplinas/${id}/renomear`, {
//         method: "POST",
//         headers: { "Content-Type": "application/json" },
//         body: JSON.stringify({ nome_disciplina: novoNome.trim() }),
//       });
//       const data = await res.json();
//       if (!res.ok) throw new Error(data.message || "Não foi possível renomear.");
//       setEditandoId(null);
//       setNovoNome("");
//       await carregar();
//     } catch (e) {
...
//         <h1 className={styles.title}>Disciplinas</h1>
//         <p className={styles.subtitle}>{disciplinas.length} disciplina(s) cadastrada(s)</p>

//         {erro && <div className={styles.erro}>{erro}</div>}

//         {disciplinas.length === 0 ? (
//           <p className={styles.vazio}>Nenhuma disciplina cadastrada.</p>
//         ) : (
//           <ul className={styles.lista}>
//             {disciplinas.map((d) => (
...
//                   ) : (
//                     <>
//                       <p className={styles.nomeDisciplina}>{d.nome_disciplina}</p>
//                       <p className={styles.meta}>{d.total_vinculos} vínculo(s) de turma/professor</p>
//                     </>
//                   )}
//                 </div>
...
//                     <button onClick={() => salvarRenomeio(d.id)} className={styles.salvarBtn}>Salvar</button>
//                     <button onClick={() => { setEditandoId(null); setNovoNome(""); }} className={styles.cancelarBtn}>Cancelar</button>
//                   </div>
//                 ) : (
//                   <button
//                     onClick={() => { setEditandoId(d.id); setNovoNome(d.nome_disciplina); }}
