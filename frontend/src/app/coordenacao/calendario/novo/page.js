...
export default function NovoEventoCoordenacaoPage() {
  ...
  const [tipoEvento, setTipoEvento] = useState("geral"); // "geral" | "especifico"
  ...
  async function handleSelecionarProfessor(professorId) {
    ...
    try {
      ...
      setErros([`Erro ao carregar turmas do professor: ${error.message}`]);
    } finally {
      setCarregandoTurmas(false);
    }
  }

  function handleTipoEventoChange(tipo) {
    setTipoEvento(tipo);
  ...
  async function handleSalvar(e) {
    ...
    if (tipoEvento === "especifico" && !form.turma_id) {
      setErros(["Selecione o professor e a turma para um evento específico."]);
      setSaving(false);
      return;
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/coordenacao/calendario/eventos/criar`, {
        ...
        body: JSON.stringify({
          ...
          descricao: form.descricao.trim(),
          data: form.data,
          turma: tipoEvento === "especifico" ? form.turma_id : null,
        }),
      });

      if (!res.ok) {
        const corpoErro = await res.text();
        let msg = `Falha ao criar evento (status ${res.status})`;
        try {
      ...
      <div className={styles.wrapper}>
        ...
        <form className={styles.form} onSubmit={handleSalvar}>
          ...
          <div className={styles.secaoDivisor}>
            <h2 className={styles.secaoTitulo}>Alcance do evento</h2>
            <p className={styles.secaoSubtitulo}>
              Escolha se o evento é geral (toda a escola) ou específico de uma turma de um professor.
            </p>
          </div>

          <div className={styles.tipoEventoRow}>
            <button
              type="button"
              className={`${styles.tipoEventoBotao} ${tipoEvento === "geral" ? styles.tipoEventoAtivo : ""}`}
              onClick={() => handleTipoEventoChange("geral")}
            >
              Evento geral
            </button>
            <button
              type="button"
              className={`${styles.tipoEventoBotao} ${tipoEvento === "especifico" ? styles.tipoEventoAtivo : ""}`}
              onClick={() => handleTipoEventoChange("especifico")}
            >
              Evento específico
            </button>
          </div>

          {tipoEvento === "especifico" && (
            <div className={styles.linhaDupla}>
              <div className={styles.campo}>
                <label className={styles.label} htmlFor="professor">
                  Professor <span className={styles.obrigatorio}>*</span>
                </label>
                <select
                  id="professor"
                  className={styles.select}
              ...
              <div className={styles.campo}>
                ...
                <select
                  ...
                  value={form.turma_id}
                  onChange={(e) => handleFormChange("turma_id", e.target.value)}
                  disabled={!professorSelecionado || carregandoTurmas}
                >
                  <option value="">
